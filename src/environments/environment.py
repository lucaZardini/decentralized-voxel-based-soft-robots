...
import numpy as np
from evogym.envs import SoftBridge as SoftBridgeEvo, WalkingFlat as WalkingFlatEvo, StepsDown as StepsDownEvo
...
class EnvironmentManager:
    ...
    def get_environment(environment_type: EnvironmentType, robot_structure: np.ndarray, robot_connections: np.ndarray):
        ...
        elif environment_type == EnvironmentType.DOWN_STEPPER:
            return DownStepper(body=robot_structure, connections=robot_connections)
        else:
...
class DownStepper(Environment):

    def __init__(self, body: np.ndarray, connections: np.ndarray):
        super().__init__(body, connections)
        self._evo_sim = StepsDownEvo(body=body, connections=connections)
